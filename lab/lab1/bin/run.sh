...
PARSER_LIB=${PARSER_DIR}/edu.ustc.cs.compile.parser.skipoominijool.jar
...
if [ $# -lt 1 ]; then
	...
	# 调用 lab1 的 Main.class
    #java -classpath ${CLASSPATH} \
	#	 edu.ustc.cs.compile.interpreter.Main \
    #     --cfg-file ../config/lab1-parser.xml

	# 调用 lab1 的 TestCase 配置文件
    #java -classpath ${CLASSPATH} \
    #     edu.ustc.cs.compile.platform.Main \
	#	 --cfg-file ../config/lab1-testcase.xml
else
    java -classpath ${CLASSPATH} \
         edu.ustc.cs.compile.platform.Main $*
fi
